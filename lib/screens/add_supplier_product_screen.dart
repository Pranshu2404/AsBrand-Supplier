...
class AddSupplierProductScreen extends StatefulWidget {
  const AddSupplierProductScreen({super.key});

  @override
  State<AddSupplierProductScreen> createState() => _AddSupplierProductScreenState();
}
...
class _AddSupplierProductScreenState extends State<AddSupplierProductScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_linkedProductId == null && (_selectedCategoryId == null || _selectedSubCategoryId == null)) {
    ...
    if (_linkedProductId == null && uploadedUrls.isEmpty && !hasSkuImages) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      // String-only fields for the provider signature
      final stringFields = <String, String>{
        'name': _nameController.text.trim(),
        'description': _descController.text.trim(),
        'price': _priceController.text.trim(),
        'quantity': _quantityController.text.trim(),
        if (_linkedProductId == null) 'proCategoryId': _selectedCategoryId!,
        if (_linkedProductId == null) 'proSubCategoryId': _selectedSubCategoryId!,
        if (_linkedProductId == null) 'gender': _selectedGender,
      };

      if (_selectedSubSubCategoryId != null) {
        stringFields['proSubSubCategoryId'] = _selectedSubSubCategoryId!;
      }
      ...
      // If we are already linking a product, bypass the duplicate check
      if (_linkedProductId != null) {
        await _executeAddProduct(stringFields, skuData, proVariants, null, _linkedProductId);
        return;
      }
      ...
      await _executeAddProduct(stringFields, skuData, proVariants, uploadedUrls.isNotEmpty ? uploadedUrls : null, null);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _executeAddProduct(
    ...
    String? selectedProductId
      ...
      setState(() => _isSubmitting = true);
      try {
        final success = await context.read<SupplierProvider>().addProduct(
          fields: stringFields,
          skuData: skuData,
        ...
        if (success && mounted) {
          ...
          final errorMsg = context.read<SupplierProvider>().error ?? 'Failed to submit product';
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
  ...
  Future<void> _checkDuplicatesEarly() async {
    if (_nameController.text.trim().isEmpty || _selectedCategoryId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter Product Name and Select Category first to search.'))
      );
      return;
    }
    setState(() => _isSubmitting = true);
    try {
      final dupRes = await context.read<SupplierProvider>().checkDuplicate(
        name: _nameController.text.trim(),
        proCategoryId: _selectedCategoryId!,
      );
      if (dupRes['success'] == true && dupRes['duplicate'] == true) {
        final suggestions = dupRes['suggestions'] as List<dynamic>? ?? [];
        if (suggestions.isNotEmpty && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No existing items found. Proceed with filling details.'))
        );
      }
    } finally {
      if (mounted) setState(() => _isSubmitting = false);
  ...
  Future<void> _showDuplicateOptionsBottomSheet(List<dynamic> suggestions) async {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'We found products in the global catalog that match your item. Link your store to an existing product to skip filling out descriptions and images!',
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                if (p['brand'] != null)
                                  Text(p['brand'], style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
                              ],
                            ),
                          ),
                          ElevatedButton(
                            onPressed: () {
                              Navigator.pop(ctx);
                              setState(() {
                                _linkedProductId = p['_id'];
                                _linkedProductName = p['name'];
                                _linkedProductImage = imgUrl;
                                // clear description/images needed when linked
                            ...
                            child: const Text('Link Item', style: TextStyle(fontSize: 12)),
                          )
                        ],
                      ),
                    );
              ...
              SizedBox(
                ...
                child: TextButton(
                  ...
                  child: const Text('No, create as new completely'),
                ),
              ),
            ...
            children: [
              ...
              if (_linkedProductId != null)
                Container(
                  ...
                  child: Row(
                    children: [
                      if (_linkedProductImage != null)
                        ClipRRect(
                          borderRadius: BorderRadius.circular(8),
                          child: Image.network(_linkedProductImage!, width: 50, height: 50, fit: BoxFit.cover),
                        ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Linked to Global Catalog', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold, fontSize: 12)),
                            Text(_linkedProductName ?? 'Product', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                          ],
                        ),
                      ),
                      IconButton(
              ...
              // ── General Images (only shown if no variants & NO linked product) ──
              if (_skus.isEmpty && _linkedProductId == null) ...[
              ...
              if (_linkedProductId == null) ...[
                ...
                // NEW: Early Check Duplicates Button
                const SizedBox(height: 12),
                SizedBox(
                  width: double.infinity,
                  child: OutlinedButton.icon(
                    onPressed: _isSubmitting ? null : _checkDuplicatesEarly,
                    icon: const Icon(Icons.search, size: 18),
                    label: const Text('Check for Existing Product to Link'),
